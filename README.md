...
输入 Bangumi 用户名 → 返回个性化动画推荐（20 条）。产品定位是**发现**：不推已看动画的续作，保证列表里有冷门作品。
...
→ 冷门配额：20 条里至少 8 条来自冷门池（热度排名 > 3000）        （ADR 0002）
   配额是"至少"语义：冷门口味用户会自然获得 >8 条冷门，主流用户保持 ~8 条
→ nsfw 保守过滤：profile 无 nsfw 口味时，候选排除黄片（纯黄片用户不受影响）
→ 按 CF 分合并排序 → top-20
```

- **冷启动**（画像不足）：回退流行度排序，仍应用配额（取冷门池最热几条）。
...
python -m scripts.evaluate_product --users 500   # 产品路径离线评估（含组成统计）
python -m scripts.measure_hotness --users 500    # 热度分布对比（纯CF/franchise/产品，验证"变冷"来源）
```

`evaluate_product.py` 测量与线上**完全一致**的路径（CF + franchise + 配额），报告 Recall/NDCG 对比 + 冷门占比 + 同系列重复数。注意：franchise 排除会系统性降低 Recall@10（因为"续作命中"被刻意赶出），这是产品要求，不是回归。

## 关键文件

| 文件 | 职责 |
|---|---|
| `src/recommender.py` | 推荐引擎深模块（评分/过滤/去重/配额）+ 共享函数 |
| `src/api.py` | FastAPI 薄层（`/v1/recommend`、`/v1/health`） |
...
关键取舍见 `docs/adr/`（0001 IDF 加权、0002 冷门配额、0003 franchise 去重）与 `docs/PLAN.md`。术语定义见 `docs/CONTEXT.md`。
