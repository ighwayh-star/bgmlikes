...
> **idf 的两次出场**（2026-08-13 实验上线）：打分矩阵 A 里的 idf 是"每部番按冷门度抬分"的乘子，实测去掉后 Recall/NDCG 大涨但热门变多（`idf_in_score=0`）；相似度 Bn/q 里的 idf 让"共看小众番"的相似度权重更高，保留有益（两处都在时比全去更好）。
...
| `idf_in_score` | 1 | 打分矩阵 A 是否带 idf 乘子；0=去掉（A 保持原分，相似度 Bn/q 的 idf 保留）。2026-08-13 实验：去掉后 Recall/NDCG 大涨但热门变多 |
